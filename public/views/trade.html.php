    ...
    <main style="display: flex; justify-content: center; align-items: flex-start; min-height: 80vh; padding-top: 50px;">
        <div class="card" style="max-width: 500px; width: 100%; padding: 40px; text-align: left;">
            ...
            <div class="trade-info" style="margin-bottom: 30px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 20px;">
                <p style="margin: 5px 0;">Aktualna cena: <strong id="price" style="color: #fff;">$<?= number_format($price, 2) ?></strong></p>
            ...
            <form action="executeTrade" method="POST">
                ...
                <div style="margin-bottom: 20px;">
                    <label style="display: block; margin-bottom: 10px; color: #888; font-size: 0.9rem;">
                        Ilość jednostek:
                        <?php if($type === 'SELL'): ?>
                            <span style="color: #00d2ff; float: right;">Posiadasz: <?= (float)$owned_amount ?></span>
                        <?php endif; ?>
                    </label>
                    ...
                    <div id="zero-error" style="color: #ff4444; font-size: 0.8rem; margin-top: 10px; display: none; font-weight: bold;">
                        <i class="fa-solid fa-triangle-exclamation"></i> Ilość musi być większa od zera!
                    </div>
                </div>
                ...
                <div id="summary-box" style="background: rgba(0,210,255,0.1); padding: 20px; border-radius: 12px; margin-bottom: 30px; border: 1px solid rgba(0,210,255,0.2);">
                    <p style="margin: 0; font-size: 0.8rem; color: #888; text-transform: uppercase; letter-spacing: 1px;">Przewidywana wartość (USD):</p>
                    <p style="margin: 5px 0 0 0; font-size: 1.8rem; font-weight: bold; color: #00ff88;">
                        $ <span id="total-value">0.00</span>
                    </p>
                </div>

                <button type="submit" id="submit-btn" style="width: 100%; padding: 20px; font-size: 1.1rem; background: <?= $type === 'BUY' ? '#00d2ff' : '#ff4444' ?>; color: <?= $type === 'BUY' ? 'black' : 'white' ?>; border: none; border-radius: 10px; font-weight: bold; cursor: pointer; transition: 0.3s; text-transform: uppercase; letter-spacing: 1px;">
    ...
    <script>
        ...
        const updateTotal = () => {
            ...
            if (tradeType === 'BUY' && total > userBalance) {
                ...
                summaryBox.style.borderColor = "rgba(255, 68, 68, 0.3)";
                totalValueSpan.style.color = "#ff4444";
            } else {
                summaryBox.style.background = "rgba(0,210,255,0.1)";
                summaryBox.style.borderColor = "rgba(0,210,255,0.2)";
